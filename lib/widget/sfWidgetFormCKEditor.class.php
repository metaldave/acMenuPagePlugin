...
class sfWidgetFormCKEditor extends sfWidgetFormTextarea {
  ...
  protected function configure($options = array(), $attributes = array())
  {
    ...
    {
    	/* codice spostato nell'invocazione del widget (nella configure delal form che lo usa)
    	// kfm
    	if (!isset($options['jsoptions']['filebrowserBrowseUrl']))
    	{
			$secret = (isset($_COOKIE['kfm_secret'])) ? $_COOKIE['kfm_secret'] : md5(time());
			setcookie('kfm_secret',$secret,0,'/');    		
    		$options['jsoptions']['filebrowserBrowseUrl'] = '/acMenuPagePlugin/js/kfm/index.php?kfm_secret='.$secret;
    	}
    	*/
    }
